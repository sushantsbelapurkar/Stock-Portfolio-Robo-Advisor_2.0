SELECT * FROM mysql_portfolio.symbol_list where exchangeShortName = 'NSE' and symbol like '%MRF%';
...
curdate() as created_at
 ...
 (
 SELECT *,
 ((power((LAST_VALUE(freeCashFlow) over(partition by symbol order by calendarYear))/(FIRST_VALUE(freeCashFlow) over(partition by symbol order by calendarYear)),(1/5)))-1)*100 as carg
 FROM free_cashflow_growth
 ),
 max_row as
 (
 ...
 free_cashflow_details.fcf_growth as recent_fcf_growth, free_cashflow_details.avg_fcf as _5yr_avg_fcf,
 round(free_cashflow_details.carg,2) as _5yr_fcf_cagr
